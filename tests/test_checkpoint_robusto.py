...
def test_falha_de_gravacao_nao_marca_a_linha_como_erro(tmp_path, parallel_requests):
    ...
    with patch('dataframeit.core._save_checkpoint', side_effect=grava_ou_falha), \
...
def test_gravacao_sempre_falhando_nao_interrompe_a_execucao(tmp_path, parallel_requests):
    _, llm = _llm_contador()
    with patch('dataframeit.core._save_checkpoint', side_effect=OSError('disco cheio')), \
            patch('dataframeit.core.call_langchain', side_effect=llm), \
            patch('dataframeit.core.validate_provider_dependencies'), \
...
def test_retomada_de_checkpoint_textual_nao_reprocessa(tmp_path, extensao):
    ...
    with patch('dataframeit.core.call_langchain', side_effect=responde), \
            patch('dataframeit.core.validate_provider_dependencies'):
        dataframeit(
    ...
    with patch('dataframeit.core.call_langchain') as call_langchain, \
            patch('dataframeit.core.validate_provider_dependencies'):
        retomado = dataframeit(
            df_lido, questions=ModeloRico, prompt='Analise {texto}', resume=True,
        )
...
def test_retomada_aceita_campo_condicional_pulado_com_tipo_obrigatorio():
    ...
    with patch('dataframeit.core.validate_provider_dependencies'), \
        ...
        resultado = dataframeit(
            df, questions=Pessoa, prompt='Analise {texto}',
            use_search=True, search_per_field=True, resume=True,
        )
...
def test_texto_obrigatorio_vazio_em_csv_continua_acusado(tmp_path):
    ...
    ckpt = tmp_path / 'ckpt.csv'
    with patch('dataframeit.core.call_langchain',
               return_value={'data': {'observacao': ''}, 'usage': None}), \
            patch('dataframeit.core.validate_provider_dependencies'):
        dataframeit(pd.DataFrame({'texto': ['x']}), questions=ComObservacao,
                    prompt='{texto}', batch_size=1, checkpoint_path=ckpt)

    with patch('dataframeit.core.validate_provider_dependencies'):
        with pytest.raises(ValueError, match='observacao'):
            dataframeit(read_df(str(ckpt), ComObservacao), questions=ComObservacao,
                        prompt='{texto}', resume=True)
...
def test_campo_condicional_ausente_com_condicao_verdadeira_e_acusado():
    ...
    with patch('dataframeit.core.validate_provider_dependencies'), \
        ...
        with pytest.raises(ValueError, match="cpf"):
            dataframeit(df, questions=Pessoa, prompt='{texto}',
                        use_search=True, search_per_field=True, resume=True)
...
def test_campo_condicional_presente_mantem_as_restricoes():
    ...
    df = pd.DataFrame({'tipo': ['pf', 'pj'], 'cpf': ['123', None],
                       '_dataframeit_status': ['processed', 'processed']})
...
def test_falha_na_ultima_gravacao_intermediaria_e_coberta_pela_final(tmp_path, parallel_requests):
    ...
    _, llm = _llm_contador()
    ckpt = tmp_path / 'ckpt.csv'
    with patch('dataframeit.core._save_checkpoint', side_effect=grava), \
            patch('dataframeit.core.call_langchain', side_effect=llm), \
            patch('dataframeit.core.validate_provider_dependencies'), \
            warnings.catch_warnings():
        warnings.simplefilter('ignore')
        dataframeit(pd.DataFrame({'texto': list('abcdef')}), questions=ModeloSimples,
                    prompt='{texto}', batch_size=2, checkpoint_path=ckpt,
                    parallel_requests=parallel_requests)
