...
_CHAVES_DA_BIBLIOTECA = (
    'condition', 'depends_on', 'prompt', 'prompt_replace', 'prompt_append',
    ...
    def test_llm_field_preserva_o_original_e_o_resto_do_extra(self):
        ...
        class Modelo(BaseModel):
            a: Optional[str] = Field(
                None, json_schema_extra={'examples': ['x'], 'condition': {'field': 'b', 'equals': 1}}
            )
        ...
        assert original.json_schema_extra == {
            'examples': ['x'], 'condition': {'field': 'b', 'equals': 1},
        }
...
class TestValidacaoAntesDeProcessar:
    ...
    def test_dependencia_circular_levanta_erro_antes_das_linhas(self):
        class Modelo(BaseModel):
            ...
            b: Optional[str] = Field(None, json_schema_extra={'condition': {'field': 'a', 'equals': '1'}})
    ...
    def test_dependencia_circular_entre_grupo_e_campo_levanta_erro(self):
        class Modelo(BaseModel):
            a: Optional[str] = Field(None, json_schema_extra={'condition': {'field': 'c', 'equals': '1'}})
            b: Optional[str] = None
            c: Optional[str] = Field(None, json_schema_extra={'condition': {'field': 'b', 'equals': '1'}})

        with pytest.raises(ValueError, match="circular"):
            _executar(
                Modelo, use_search=True, search_per_field=True,
    ...
    @pytest.mark.parametrize('opcoes, falta', [
...
class TestTextoNoPrompt:
    ...
    @pytest.mark.parametrize('prompt_do_grupo', [
        'Busque: {query}',
        'Busque o contexto',
        'Busque: {texto}',
    ])
...
def test_reprocess_columns_por_campo_chama_so_os_campos_pedidos(parallel_requests, search_groups):
    ...
    with provider, busca, patch('dataframeit.agent.call_agent', side_effect=call_agent):
        resultado = dataframeit(
            ...
            parallel_requests=parallel_requests, search_groups=search_groups,
        )
