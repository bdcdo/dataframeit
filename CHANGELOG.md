...
- Os pisos das dependências passam a ser versões que resolvem e passam na suíte: `langchain>=1.3.0`, `langchain-core>=1.4.7`, `pydantic>=2.11.0`, `pandas>=2.2.2`, `tqdm>=4.0`, `pyarrow>=16`, `langchain-openai>=0.3.34`, `langchain-anthropic>=0.3.21`, `langchain-google-genai>=2.1.11`, `langchain-groq>=1.0.0`, `langchain-tavily>=0.2.12` e `langchain-exa>=1.0.0`. Os anteriores aceitavam versões que não instalam juntas ou que quebram no import. Um job de CI roda a suíte com cada dependência direta no piso (#130).
