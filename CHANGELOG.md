...
- O provider padrão passa a ser `openai`, e `model` passa a ter default `None`: sem `model`, cada provider usa o próprio modelo padrão (`DEFAULT_MODELS`: `gpt-6-luna` na OpenAI, `gemini-3.8-flash` no Google, `claude-sonnet-5` na Anthropic, `openai/gpt-oss-120b` na Groq). Providers fora dessa tabela exigem `model`, e `codex` e `claude_code` deixam a escolha ao runtime.
- Documentação, READMEs e notebooks de exemplo atualizados para os modelos atuais; saem modelos desligados ou restritos (Gemini 2.5 e `gemini-3-flash-preview`, Llama e `groq/compound` na Groq, `claude-3-5-sonnet`).
